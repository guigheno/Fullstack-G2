...
async function list(req,res,next){
  try {
    const paging = getPagingAndSorting(req.query, {
      defaultPageSize: 10, maxPageSize: 100, allowedSortBy: ['id','name','email','createdAt']
    });
...
async function remove(req,res,next){ try{ await service.remove(Number(req.params.id)); res.json({message:'Cliente removido com sucesso.'}); }catch(e){ next(e); } }
